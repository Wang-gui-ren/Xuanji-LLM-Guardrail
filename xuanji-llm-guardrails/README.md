...
项目参考 `STAIR-LLMGuardrails` 的核心思路，为 AstrBot 提供输入护栏、输出护栏、工具链防护、运行时审计、本地安全知识沉淀与安全演化能力。插件遵循“尽量不侵入主框架”的原则，主要通过 AstrBot 插件机制接入，而不是修改核心组件。

当前插件包标识仍为 `astrbot_plugin_senbenzakula_guard`，以保持现有安装目录、数据目录和兼容性稳定；AstrBot 对外展示名与 GitHub 项目名称统一为“璇玑大模型护栏 | Xuanji LLM Guardrails”。
...
它的定位更接近“本地安全知识记录系统”，而不是完整的外部知识图谱或独立向量数据库。
...
本插件的重点不在于替代 AstrBot 自身能力，而在于为 AstrBot 增加一层可独立演化的安全护栏框架，使输入、输出、工具链与本地知识沉淀形成可持续优化的闭环。
